...
from game_logging.metrics_calculator import calculate_round_metrics, calculate_game_metrics

# Property 1: Round outcome determination
@given(
    winner_is_spy=st.booleans(),
    spy_guessed=st.booleans()
)
...
@given(
    questions=st.lists(st.text()),
    answers=st.lists(st.text())
)
...
def test_game_metrics_property(rounds):
    ...
    for winner_is_spy, spy_guessed in rounds:
        ...
        rs.votes = []
        rs.conversation_history = [Turn(1, "A", "B", "Q", "A")] # 1 turn per round for simplicity
