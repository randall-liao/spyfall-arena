...
from game_logging.metrics_calculator import RoundMetrics, GameMetrics, calculate_round_metrics, calculate_game_metrics
...
def test_calculate_round_metrics_spy_wins_wrong_vote():
    ...
    successful_vote = VoteAttempt(initiator="Alice", suspect="Bob", votes={}, passed=True)
...
def test_calculate_round_metrics_spy_wins_location_guess():
    ...
    rs.spy_guess = SpyGuess(spy_nickname="SpyPlayer", guessed_location="Loc", actual_location="Loc", correct=True)
    rs.conversation_history = []
...
def test_calculate_round_metrics_civilians_win():
    ...
    successful_vote = VoteAttempt(initiator="Alice", suspect="SpyPlayer", votes={}, passed=True)
    rs.votes = [successful_vote]
    rs.conversation_history = []
    rs.role_assignments = {
        "SpyPlayer": Role(is_spy=True, location=None),
...
def test_calculate_round_metrics_vote_accuracy():
    ...
    vote = VoteAttempt(
        initiator="Alice", 
        suspect="SpyPlayer", 
        votes={"Alice": True, "Bob": True, "Charlie": False, "SpyPlayer": False}, # Charlie is Civ, but voted No
        passed=True
    )
    rs.votes = [vote]
    rs.role_assignments = {
...
def test_calculate_round_metrics_no_successful_vote():
    rs = MagicMock(spec=RoundState)
    rs.spy_nickname = "SpyPlayer"
    rs.votes = [VoteAttempt(initiator="A", suspect="B", votes={}, passed=False)] # Failed vote
    rs.conversation_history = []
...
def test_calculate_round_metrics_response_stats():
    ...
    t1 = Turn(turn_number=1, asker_nickname="A", answerer_nickname="B", question="QQQ", answer="AAA") # 3, 3
    t2 = Turn(turn_number=2, asker_nickname="B", answerer_nickname="A", question="Q", answer="AAAAA") # 1, 5
...
def test_calculate_game_metrics_avg_turns():
    ...
    r2 = MagicMock(spec=RoundState)
    r2.conversation_history = [Turn(1,"","", "", ""), Turn(2,"","", "", "")] # 2 turns
    r2.spy_nickname = "S"
    r2.votes = []
    r2.spy_guess = None
